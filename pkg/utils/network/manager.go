...
func (m *Manager) scanInterfaces() error {
	...
	for _, iface := range interfaces {
		...
		info.Masks = make([]net.IPMask, 0)
		for _, addr := range addrs {
			switch v := addr.(type) {
			case *net.IPNet:
				info.Addresses = append(info.Addresses, v.IP)
				info.Masks = append(info.Masks, v.Mask)
			case *net.IPAddr:
				info.Addresses = append(info.Addresses, v.IP)
				info.Masks = append(info.Masks, nil)
			}
		}
...
// IsMulticastSupport 判断指定接口是否支持多播（已启用、支持多播、非回环且有IP地址）
func (m *Manager) IsMulticastSupport(name string) (bool, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()

	iface, exists := m.interfaces[name]
	if !exists {
		return false, fmt.Errorf("未找到接口 %s", name)
	}
	...
	return iface.Flags&net.FlagUp != 0 &&
		iface.Flags&net.FlagMulticast != 0 &&
		iface.Flags&net.FlagLoopback == 0 &&
		len(iface.Addresses) > 0, nil
}

// CalculateIPv4Broadcast 计算IPv4的广播地址（返回广播地址）
func CalculateIPv4Broadcast(ip net.IP, mask net.IPMask) (net.IP, bool) {
	// 仅处理IPv4地址
	if ip.To4() == nil {
		return nil, false
	}
...
// GetInterfaceBroadcastAddresses 获取指定接口的所有IPv4广播地址
// 一个接口可能有多个IP（如多个IPv4地址），每个IP对应一个广播地址
func (m *Manager) GetInterfaceBroadcastAddresses(name string) ([]net.IP, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
	...
	// 遍历接口的所有子网，计算每个IPv4对应的广播地址
	for i, mask := range iface.Masks {
		if broadcast, ok := CalculateIPv4Broadcast(iface.Addresses[i], mask); ok {
