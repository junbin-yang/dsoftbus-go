...
func TestDeriveSessionKey(t *testing.T) {
	sharedSecret := []byte("shared_secret_data_for_testing_purposes")
	salt := []byte("salt_value")
	keyLen := SessionKeyLength

	sessionKey, err := deriveSessionKey(sharedSecret, salt, keyLen)
	if err != nil {
	...
	sessionKey2, err := deriveSessionKey(sharedSecret, salt, keyLen)
	if err != nil {
		t.Fatalf("派生会话密钥失败: %v", err)
	}
	...
	sessionKey3, err := deriveSessionKey([]byte("different_secret"), salt, keyLen)
	if err != nil {
		t.Fatalf("派生会话密钥失败: %v", err)
	}
...
func TestHiChainAuthenticationFlow(t *testing.T) {
	...
	// 2. 接收方处理认证开始消息
	err = peerHandle.handleAuthStart(startMsg, transmittedData)
	if err != nil {
